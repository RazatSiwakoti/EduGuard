...
interface UserManagementHooks {
  useList: (includeInactive: boolean) => {
    data: User[] | undefined;
    isLoading: boolean;
    isError: boolean;
  };
  ...
  useDeactivate: () => { mutate: (id: number) => void };
  useReactivate: () => { mutate: (id: number) => void };
...
export default function UserManagementPanel({
  ...
  const { data: users, isLoading, isError } = hooks.useList(showInactive);
  const create = hooks.useCreate();
  const deactivate = hooks.useDeactivate();
  const reactivate = hooks.useReactivate();
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-md border border-stone-200 bg-white">
        ...
        {users && users.length === 0 && (
          <p className="p-6 text-sm text-stone-500">{emptyStateLabel}</p>
        )}
        {users && users.length > 0 && (
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-stone-100 last:border-0">
                  <td className="px-4 py-2.5 text-stone-900">{u.full_name}</td>
